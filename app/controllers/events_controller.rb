class EventsController < ApplicationController
 ...
 # before_action :require_owner_event, only: [:edit, :update, :destroy]
  before_action :urbanist, only: [:event_now]
  before_action :verified_venues, only: [:event_now, :index,:landing,:urbanist, :downtown,:shadyside, :south_side, :lawrenceville, :oakland, :north_side, :bloomfield, :east_liberty, :strip_district, :squirrel_hill, :mt_washington]
 ...
 def shadyside
   ...
   @neighborhood_tag = 2
    ...
    @daily_specials.each do |feature|
        feature_increase = (feature.credit*increase_array[rand(0..9)])/(feature.credit + (current_date  - feature.created_at.to_date).to_i + 1)
  ...
  def downtown
   ...
   @events = @events_all.select{|event|  venue_id.include?(event.venue_id)}.select {|event| @day_specials.include?(event.day)}.sort! {|x,y| (y.cached_votes_total + y.credits - y.venue.avg_verify) <=> (x.cached_votes_total + x.credits - x.venue.avg_verify)}
   @tag_events = Event.where(venue_id: venue_id, day: @day_specials)
   @tag_topic = ""
